...
def create_archive(source: Path, output: Path, excluded_names: set[str]) -> None:
        ...
        with gzip.GzipFile(filename="", mode="wb", fileobj=raw, mtime=0) as compressed:
            with tarfile.open(fileobj=compressed, mode="w", format=tarfile.GNU_FORMAT) as archive:
                for path in _paths(source, excluded_names):
                    ...
                    archive_name = source.name if relative == Path(".") else str(Path(source.name) / relative)
...
def main() -> None:
    ...
    create_archive(args.source, args.output, set(args.exclude))
