...
def test_candidate_fingerprint_is_required_and_can_bind_the_installed_binary(tmp_path, monkeypatch):
    ...
    with info_path.open("wb") as handle:
        plistlib.dump({
            ...
            "CFBundleShortVersionString": "0.14.1",
...
def test_release_decision_accepts_only_a_bounded_voice_waiver():
    assert validate_release_decision(
        release_decision_payload(), expected_version="0.14.1"
    ) == ("passed", None)
    ...
    assert "no_full_coverage_claim" in validate_release_decision(
        overclaim, expected_version="0.14.1"
    )[1]
    ...
    assert "audio or transcript" in validate_release_decision(
        private, expected_version="0.14.1"
    )[1]
...
def test_acceptance_runner_bootstraps_clean_repositories_and_accepts_worktrees():
    script = (ROOT / "scripts/run_vnext_single_release_acceptance.sh").read_text(encoding="utf-8")
    assert 'if [[ ! -e "$repo/.git" ]]' in script
    setup = script.index("acceptance_environment_setup")
    orchestrator = script.index("run_gate orchestrator_check")
    assert setup < orchestrator
    assert "prepare_vnext_acceptance_environment.sh" in script

    bootstrap = (ROOT / "scripts/prepare_vnext_acceptance_environment.sh").read_text(
        encoding="utf-8"
    )
