...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("across-autopilot", "@across/autopilot", "0.5.0", run_autopilot_plugin_lifecycle_action),
    ],
)
...
def test_one_click_orchestrator_install_uses_native_payload_and_preserves_data(monkeypatch, tmp_path):
    ...
    native = _write_executable(
        payload_root / "runtimes" / "orchestrator-0.10.3" / "across-orchestrator",
        "#!/bin/sh\n"
        "case \"$1\" in\n"
        "  plugin-manifest) printf '{\"id\":\"across-orchestrator\",\"displayName\":\"Across Orchestrator\",\"kind\":\"task-runtime\",\"version\":\"0.10.3\"}\\n' ;;\n"
        "  plugin-status) printf '{\"status\":\"installed\",\"installed\":true,\"available\":true}\\n' ;;\n"
    ...
    assert status["source"] == "bundle://across-orchestrator/0.10.3"
