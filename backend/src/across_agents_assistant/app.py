...
from .paths import log_dir as app_log_dir, tmp_dir as app_tmp_dir
...
class AcrossAgentsAssistantApp:
    def __init__(self, project_root: Path, config: AppConfig):
        ...
        self._local_agent = OrchestratorClient(config_manager=self._agent_manager)
        self._tts = TTSService(temp_dir=app_tmp_dir() / "tts")

        self._llm_gateway = get_gateway()  # LLM Gateway for task planning

        # Task Manager for multi-agent coordination
        self._task_state = TaskState()
        self._task_dispatcher = TaskDispatcher(self._task_state, self._local_agent)
                ...
                try:
                    if AGENTS_CONFIG_FILE.exists():
                        ...
                        if self._last_config_mtime != 0 and mtime > self._last_config_mtime:
                            ...
                            if getattr(self._local_agent, 'agent_id', None) != new_active:
                                self._logger.info(f"🔄 检测到配置变化，热切换到智能体: {new_active}")
                                self._local_agent.initialized = False
                                self._local_agent.initialize()
    ...
    def _worker_loop(self):
        ...
        # threading.Thread(target=self._local_agent.initialize, daemon=True).start()
    ...
    def _process_agent_reply(self, text: str, target_agent: str):
        try:
            ...
            # Use persistence for history just like api_server
            ...
            persistence.add_message(session_id=session_id, role="user", content=text)
            messages = persistence.get_messages(session_id)

            # Format messages for Orchestrator
            formatted_messages = []
            system_msg = "You are a helpful AI assistant running in a macOS desktop environment. You are NOT Claude. You are NOT Hermes. You are NOT OpenClaw. You are the Across Agents Assistant, a versatile tool for macOS users. Keep your answers brief and suitable for voice synthesis."
