    ...
    if (needs(/\broute\s+evidence|recompute\s+route\b/)) {
      const routeResult = await page.evaluate(async () => {
        ...
        return {
          ...
          hasSelectedAgent: lowered.includes('selected agent') || /(openclaw|hermes|claude code|cloudcode desktop|codex|opencode|cursor agent|openai|anthropic|deepseek|minimax|agnes|bailian|moonshot|zhipu|volcengine|google|xai|mistral|groq|cohere|openrouter|together|fireworks)/.test(lowered),
