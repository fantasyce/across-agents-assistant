...
from across_agents_assistant.paths import app_subdir
...
def _resolve_release_e2e_project_dir(project_dir: Optional[str], run_label: Optional[str]) -> Path:
    raw_text = str(project_dir or _default_project_dir(run_label)).strip()
    if not raw_text:
        raise ValueError("Release E2E project_dir must not be empty")
    expanded_text = os.path.expanduser(raw_text)
    if not os.path.isabs(expanded_text):
        raise ValueError("Release E2E project_dir must be an absolute path")
    normalized_text = os.path.realpath(expanded_text)
    home_text = os.path.realpath(os.path.expanduser("~"))
    root_text = os.path.realpath(os.path.sep)
    if normalized_text == home_text or normalized_text == root_text:
        raise ValueError("Release E2E project_dir cannot target a home or filesystem root directory")
    allowed_roots = [
        os.path.realpath(tempfile.gettempdir()),
        os.path.realpath(str(app_subdir("orchestrator-workspaces"))),
    ]
    if not any(_is_relative_to(normalized_text, root) for root in allowed_roots):
        raise ValueError(
            "Release E2E project_dir must be under the system temp directory "
            "or the managed Across Agents Assistant workspace directory"
        )
...
def build_release_e2e_task_request(
    *,
    ...
    project_path = _resolve_release_e2e_project_dir(project_dir, run_label)
