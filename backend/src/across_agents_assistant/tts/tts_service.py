...
MOSS_TTS_PATH = os.environ.get("MOSS_TTS_PATH")
MOSS_TTS_MODEL_DIR = os.environ.get("MOSS_TTS_MODEL_DIR")

if MOSS_TTS_PATH:
    moss_tts_path = os.path.expanduser(MOSS_TTS_PATH)
...
class TTSService:
    def __init__(self, temp_dir: Path):
        ...
        if OnnxTtsRuntime is not None and MOSS_TTS_MODEL_DIR:
            try:
                self._moss_runtime = OnnxTtsRuntime(
                    model_dir=os.path.expanduser(MOSS_TTS_MODEL_DIR),
                    thread_count=4,
                    max_new_frames=1000,
                    do_sample=True,
                    sample_mode="fixed",
    ...
    def _generate_wav_moss(self, text: str, output_path: Path):
        if not MOSS_TTS_PATH:
            raise RuntimeError("MOSS_TTS_PATH is required for MOSS-TTS-Nano voice prompts.")

        # We use zh_6.wav as the default voice for Nano
        voice_path = os.path.join(os.path.expanduser(MOSS_TTS_PATH), "assets", "audio", "zh_6.wav")
